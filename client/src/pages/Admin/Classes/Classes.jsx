...
export default function Classes()
{
    ...
    const [curr_class, setCurr] = useState(classList[0]);
    function handleShowInfo(p_class)
    {
        setShowInfo(true);
        setCurr(p_class);
    }
            ...
            <div className='main-container'>
                <div className='entity-box' display="none">
                    ...
                    <div className='entity-list-container'>
                        {
                            classList.filter((iclass) =>
                            {
                                return iclass.Name.toLowerCase().match(searchInput.toLowerCase());
                            }).map((i_class, index) => (
                                <ClassDetails
                                    key={ index }
                                    iclass={ i_class }
                                    onInfo={ () => handleShowInfo(i_class) }
                                />
                            ))
                        }
                    </div>
...
function ClassDetails({ iclass, onInfo })
{
    return (
        <div className='entity-container'>
            ...
            <button class="btn btn-primary" onClick={ () => { window.location.href = `/Classes/${ iclass.Name }` } }>Details</button>
        </div>
    )
}